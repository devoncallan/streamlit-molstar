...
const Molstar = (props) => {
  ...
  useEffect(() => {
    async function update() {
      ...
      const query = compileIdListSelection("C 306-311", "auth")
      const b = plugin.current.managers.structure.selection.fromCompiledQuery(
        "add",
        query
      )
      console.log(query)
      ...
      //   (Q) =>
      //     Q.struct.generator.atomGroups({
      //       "chain-test": Q.core.rel.eq(["B", Q.ammp("label_seq_id")]),
      //     }),
      //   // plugin.current.managers.structure.hierarchy.selection.structures
      //   structures[0]
      // )
